...
namespace Business.Handlers.GroupClaims.Queries
{
    ...
    {
        public class  GetGroupClaimsQueryHandler(IGroupClaimRepository groupClaimRepository) : IRequestHandler<GetGroupClaimsQuery, IDataResult<IEnumerable<GroupClaim>>>
        {
            ...
            public async Task<IDataResult<IEnumerable<GroupClaim>>> Handle(GetGroupClaimsQuery request, CancellationToken cancellationToken) => 
                new SuccessDataResult<IEnumerable<GroupClaim>>(await _groupClaimRepository.GetListAsync());
        }
    }
}