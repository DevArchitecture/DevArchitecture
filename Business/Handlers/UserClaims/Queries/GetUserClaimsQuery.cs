...
namespace Business.Handlers.UserClaims.Queries
{
    ...
    {
        public class
            GetUserClaimsQueryHandler(IUserClaimRepository userClaimRepository) 
            : IRequestHandler<GetUserClaimsQuery, IDataResult<IEnumerable<UserClaim>>>
        {
            ...
            public async Task<IDataResult<IEnumerable<UserClaim>>> Handle(GetUserClaimsQuery request, CancellationToken cancellationToken) 
                => new SuccessDataResult<IEnumerable<UserClaim>>(await _userClaimRepository.GetListAsync());
        }
    }
}